...
class UserManager(BaseUserManager):
    ...
    def create_vendor(
        self, email, password=None, is_staff=False, is_active=True,is_vendor=True, **extra_fields
    ):
        """Create a user instance with the given email and password."""
        email = UserManager.normalize_email(email)
        # Google OAuth2 backend send unnecessary username field
        ...
        user = self.model(
            email=email, is_active=is_active, is_staff=False, is_vendor=True, **extra_fields
        )
        if password:
            user.set_password(password)
        user.save()
    ...
    def vendor(self):
        return self.get_queryset().filter(vendor=True)
